...
function generateContractContent(
  ...
  const replacements = {
    ...
    "{importer.representative.name}":
      contactData.name ||
      quotation.request.company.contactName ||
      "_________________",
    "{importer.ci}": contactData.ci || "_________________",
    "{beneficiary.name}":
      quotation.request.provider?.name || "_________________",
    "{provider.name}": quotation.request.provider?.name || "_________________",
    "{provider.bankName}":
      banking.bankName ||
      quotation.request.provider?.bankingDetails?.bankName ||
      "_________________",
    "{provider.accountNumber}":
      banking.accountNumber ||
      quotation.request.provider?.bankingDetails?.accountNumber ||
      "_________________",
    "{provider.swiftCode}":
      banking.swiftCode ||
      quotation.request.provider?.bankingDetails?.swiftCode ||
      "_________________",
    "{provider.beneficiaryName}":
      banking.beneficiaryName ||
      quotation.request.provider?.bankingDetails?.beneficiaryName ||
      "_________________",
    "{provider.bankAddress}":
      banking.bankAddress ||
      quotation.request.provider?.bankingDetails?.bankAddress ||
      "_________________",
    ...
    "{service.amountWords}": numberToWords(Number(quotation.amount) || 0),
    "{service.amount}": (Number(quotation.amount) || 0).toLocaleString(),
    "{service.feeWords}": numberToWords(
      Math.round((Number(quotation.amount) || 0) * 0.05)
    ),
    "{service.fee}": Math.round(
      (Number(quotation.amount) || 0) * 0.05
    ).toLocaleString(),
    "{contract.startDate}": formatDate(startDate.toISOString()),
    "{contract.endDate}": formatDate(endDate.toISOString()),
    "{contract.date}": formatDate(new Date().toISOString()),
  };

  const contractTemplate = `DOCUMENTO PRIVADO
...
// POST: Automatically create a contract from an accepted quotation
export async function POST(request: NextRequest) {
  try {
    ...
    // Load quotation with all relations
    ...
    // Calculate contract dates (6 months from now)
    const startDate = new Date();
    const endDate = new Date();
    endDate.setMonth(endDate.getMonth() + 6);

    // Extract CI from CARNET_IDENTIDAD document
    const carnetDocument = quotation.request.company.documents.find(
      (doc) => doc.documentInfo
    );
    const ci = carnetDocument?.documentInfo || "";

    // Prepare additional data with all the information from the contract preview
    const additionalData = {
      companyData: {
        ...
        activity: quotation.request.company.activity,
        companyType: quotation.request.company.companyType,
        contactName: quotation.request.company.contactName,
        contactPosition: quotation.request.company.contactPosition,
        bankingDetails: quotation.request.company.bankingDetails,
      },
      contactData: {
        name:
          quotation.request.createdBy.firstName +
          " " +
          quotation.request.createdBy.lastName,
        position: quotation.request.company.contactPosition,
        phone: quotation.request.company.phone,
        email: quotation.request.company.email,
        ci: ci, // Extracted from CARNET_IDENTIDAD document
      },
      providerData: {
        ...
        additionalInfo: quotation.request.provider.additionalInfo,
      },
      requestData: {
        code: quotation.request.code,
        description: quotation.request.description,
        amount: quotation.request.amount,
      ...
      quotationData: {
        ...
        managementServiceBs: quotation.managementServiceBs,
        managementServicePercentage: quotation.managementServicePercentage,
        correspondentBankBs: quotation.correspondentBankBs,
        correspondentBankUSD: quotation.correspondentBankUSD,
        swiftBankBs: quotation.swiftBankBs,
        swiftBankUSD: quotation.swiftBankUSD,
    ...
    const initialContent = generateContractContent(
      quotation,
      additionalData,
      startDate,
      endDate
    ...
    // Create the contract with all required information
    const newContract = await prisma.contract.create({
      data: {
        code: contractCode,
        title: `Contrato de Servicio - ${quotation.request.company.name}`,
        description: `Contrato de servicios de importación basado en la cotización ${quotation.code} para la solicitud ${quotation.request.code}`,
        amount: quotation.amount,
